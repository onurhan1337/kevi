...
Kevi is a **multi-tenant edge-native key-value API with identity-first architecture** built on Cloudflare Workers and KV. It provides a secure API layer for managing dynamic configurations with automatic service isolation, role-based access control, and prefix-based key management.
...
The following sequence diagram shows how a request flows through Kevi from client to KV storage:
...
The following text diagram illustrates the complete request lifecycle:
